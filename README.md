...
* **Worker de Inteligência de Dados — Python:** Serviço isolado responsável por rodar rotinas de ETL (*Cron Jobs*). O ecossistema Python carrega o dataset oficial estruturado da EA Sports (`ea_fc26_players.csv`) via `pandas` e executa o motor matemático (Similaridade Vetorial e Distância Euclidiana) para calcular os sósias de estilo de jogo ("O Novo Zidane") antes de salvar os resultados no banco — liberando a API Core de processamentos pesados.
  * *Ponto de manutenção — Ingestão:* O carregamento do CSV oficial e o mapeamento para o schema do MongoDB ficam concentrados em `data-worker/src/main.py`. A coluna `id` do CSV é preservada como `eaPlayerId`, chave usada pelo frontend para montar a URL da foto no CDN oficial.
* **Otimização de Mídia (Imagens e Fotos):** Para manter o banco de dados leve e evitar custos de armazenamento em nuvem, o aplicativo consome as fotos diretamente do CDN oficial da EA, interpolando o `eaPlayerId` de cada jogador.
