...
### Backend, Infraestrutura & Estratégia de Dados (Arquitetura de Microsserviços Híbrida)

Adotamos uma abordagem de microsserviços conteinerizados para garantir alta coesão, baixo acoplamento e facilitar testes de estresse (carga e throughput).

* **API Core — NestJS (TypeScript):** Responsável por servir os dados ao aplicativo mobile de forma ultra-rápida. O uso do NestJS mantém a tipagem estática e as interfaces alinhadas com o frontend em Angular, garantindo um ecossistema TypeScript unificado na camada de transporte de dados.
* **Worker de Inteligência e Scraping — Python:** Serviço isolado responsável por rodar rotinas de madrugada (*Cron Jobs*). O ecossistema Python lida com a raspagem complexa da fonte externa e executa o motor matemático (Similaridade Vetorial e Distância Euclidiana) para calcular os sósias de estilo de jogo ("O Novo Zidane") antes de salvar os resultados no banco — liberando a API Core de processamentos pesados.
  * *Ponto de manutenção — Seletor da tabela:* O scraper localiza os jogadores via `soup.find("table", class_=re.compile(r"\btable\b"))` no arquivo `data-worker/src/scraper_engine.py`. Se o sofifa.com alterar o nome da classe CSS ou a estrutura do HTML, **este é o único ponto que precisa ser ajustado**. O restante do pipeline (controle de estado, motor matemático, upsert no MongoDB) permanece intacto.
* **Otimização de Mídia (Imagens e Fotos):** Para manter o banco de dados leve e evitar custos de armazenamento em nuvem, o aplicativo consome as fotos via link original (*hotlinking*).
  ...
  * *Image Proxy Shield:* Para mitigar proteções de segurança de origem da fonte externa (Erros 403 / Referer Header), a API Core disponibiliza uma rota de proxy ultraleve que mascara a requisição da imagem e a repassa de forma limpa para o aplicativo móvel.
...
* **Orquestração:** Todo o ecossistema (Frontend, API NestJS, Worker Python, MongoDB e PostgreSQL) é orquestrado via **Docker**, isolando o consumo de CPU/RAM de cada serviço e garantindo estabilidade absoluta do desenvolvimento à produção.
